      ...
      document.addEventListener(REMOVE_EVENT, function () {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            Swal.fire({
              title: "Hapus Buku",
              text: "Kamu berhasil menghapus buku!",
              icon: "success",
            });
            const parentElement = event.target.parentElement.parentElement;
            DeleteAnItem(parentElement);
            const bookData = GetBookList();
            ResetAllForm();
            RenderBookList(bookData);
          }
...
function SearchBookList(title) {
  ...
  for (let index = 0; index < bookData.length; index++) {
    ...
    if (
      bookData[index].title.includes(title) ||
...
document.addEventListener(SAVED_EVENT, function () {
  Swal.fire({
    ...
    text: "Kamu berhasil menyimpan data buku!",
    icon: "success",
    confirmButtonText: "OK",
  });
});

document.addEventListener(LOAD_EVENT, function () {
  Swal.fire({
    title: "Load Data sebelumnya",
    text: "Kamu berhasil memuat data buku sebelumnya!",
    icon: "info",
    confirmButtonText: "OK",
  });
});
